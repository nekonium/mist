...
let menuTempl = function(webviews) {
  ...
  if (process.platform === 'darwin' || process.platform === 'win32') {
    ...
    if (gethClient) {
      nodeSubmenu.push({
        label: `Nekonium ${gethClient.version}`,
    ...
    if (ethClient) {
      // TODO C++ Version of Nekonium?
      nodeSubmenu.push({
        label: `NekoniumC ${ethClient.version} (C++)`,
  ...
  helpMenu.push(
    ...
    // },
    {
      ...
      click() {
        shell.openExternal('https://github.com/nekonium/mist/issues');
      }
    }
  );
