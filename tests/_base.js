...
const startGeth = function*() {
  ...
  if (!manager.clients.Geth.state.available) {
      ...
      const downloadedGeth = yield manager.download('Gnekonium');
  ...
  console.info('Gnekonium starting...');
  yield geth.start();
  console.info('Gnekonium started');
...
exports.mocha = (_module, options) => {
  ...
  _module.exports[options.name || path.basename(_module.filename)] = {
    *before() {
      ...
      // TODO Rename Mist?
      const appFileName = options.app === 'wallet' ? 'Nekonium Wallet' : 'Mist';
      const platformArch = `${process.platform}-${process.arch}`;
      console.info(`${appFileName} :: ${platformArch}`);

      let appPath;
      const ipcProviderPath = path.join(this.geth.dataDir, 'gnekonium.ipc');
    ...
    *after() {
      ...
      if (this.geth && this.geth.isRunning) {
        console.log('Stopping gnekonium...');
...
const Utils = {
  ...
  *getRealAccountBalances() {
    ...
    let balances = accounts.map(
      acc => `${this.web3.fromWei(this.web3.eth.getBalance(acc), 'nuko')}`
    );
