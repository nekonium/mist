      ...
      if (type === 'ethersale') {
        ...
        if (ethereumNode.isEth) {
          ...
          if (process.platform === 'darwin')
            keystorePath += '/Library/Nekonium/keystore';
          ...
          if (
            ...
            keystorePath += '/.nekonium/keystore';

          if (process.platform === 'win32')
            keystorePath = `${Settings.appDataPath}\\Nekonium\\keystore`;
        }
...
ipc.on('backendAction_importWalletFile', (e, path, pw) => {
  ...
  nodeProcess.once('error', () => {
    ...
    e.sender.send(
      ...
      'Couldn\'t start the "gnekonium wallet import <file.json>" process.'
    );
  });
