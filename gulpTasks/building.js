...
gulp.task('bundling-interface', (cb) => {
    ...
    if (type === 'wallet') {
        if (options.walletSource === 'local') {
            ...
            console.log(`Pulling https://github.com/nekonium/meteor-dapp-wallet/tree/${options.walletSource} "${options.walletSource}" branch...`);
            bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/nekonium/meteor-dapp-wallet.git \
...
gulp.task('build-dist', (cb) => {
    const appPackageJson = _.extend({}, require('../package.json'), {  // eslint-disable-line global-require
        ...
        homepage: 'https://github.com/nekonium/mist',
        build: {
            appId: `org.nekonium.${type}`,
            asar: true,
            directories: {
